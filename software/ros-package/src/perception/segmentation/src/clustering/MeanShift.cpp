...
void MeanShift::meanshift(const std::vector<Point> &points,
...
{
    labels.resize(shifted_points.size());
    clusters.clear();
...
void MeanShift::cluster(const std::vector<Point> &points, double kernel_bandwidth,
                        vector<Cluster> &clusters,std::vector<int> &labels ){
    vector<Point> shifted_points;
    CLUSTER_EPSILON = kernel_bandwidth * 0.0001;
    meanshift(points, shifted_points, kernel_bandwidth);
    cluster(points, shifted_points, clusters, labels);
}
